...
- 优化 Repository 查询方法，提升查询性能
- 优化归还消息提示，根据罚款原因（逾期/损失）生成详细提示消息
- 优化图书归还成功消息格式
...
- 改进删除操作的错误处理，统一所有删除操作的错误处理逻辑
- 返回明确的错误信息而不是 500 错误
- 添加删除前的关联数据检查
- 在相关 Repository 中添加 countBy 查询方法

#### 测试

- 更新 BorrowServiceTest 以适配新的业务逻辑
- 更新 FineServiceTest 以适配新的方法签名
...
- 增强罚款管理功能
  - 添加罚款记录查询接口
  - 优化 payFine 方法参数从 readerId 改为 readerNo
- 实现读者类别和作者管理功能
  - 完善读者类别和作者的增删改查功能
  - 添加删除前的关联数据检查
  - 在 BookRepository 中添加 countByAuthorId 方法
...
- 添加借阅记录查询和统计功能
  - 新增 BorrowRecordController 支持借阅记录分页查询
  - 新增 StatisticsController 提供仪表盘统计数据
  - 在 BorrowRecordRepository 中添加分页查询方法
...
- 将测试数据从 library.sql 分离到 test.sql
  - 便于区分生产数据和测试数据
